...
import { getDemoAdminStats, isDemoModeEnabled } from "@/lib/demo";

export default async function AdminPage() {
  const stats = isDemoModeEnabled()
    ? getDemoAdminStats()
    : await (async () => {
        ...
        return { totalSales, paidPayments, activeLoans, pendingOrders };
      })();
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-3">
        <Card className="rounded-3xl border-0 shadow-sm">
          ...
          <CardContent className="text-2xl font-bold">{formatRupiah(stats.totalSales)}</CardContent>
        </Card>
        <Card className="rounded-3xl border-0 shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-base">Pembayaran Berhasil</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-bold">{formatRupiah(stats.paidPayments)}</CardContent>
        </Card>
        <Card className="rounded-3xl border-0 shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="text-base">Pinjaman Aktif</CardTitle>
          </CardHeader>
          <CardContent className="text-2xl font-bold">{stats.activeLoans}</CardContent>
        </Card>
        <Card className="rounded-3xl border-0 shadow-sm">
          <CardHeader className="pb-2">
